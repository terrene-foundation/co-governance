...
> **Implementation status (read first) — mode-scoped (see SKILL.md mode guard, GATE-B).** The shapes below are the on-disk shapes the engine reads and writes in BOTH modes — `posture.json` and `violations.jsonl` are seeded and live in this hooks-only/advisory repo. In a **full-enforcement** repo the adjudication/downgrade behavior over those shapes is additionally LIVE (GH #16 — `rules/trust-posture.md` § Enforcement Status) and pinned by the posture-engine fixture suite, which is not present here. Two designed components were deliberately excluded from the GH #16 engine build (issue re-scope trail, 2026-06-10) and have NO mechanical implementation in either mode: the `stop.js` self-confession scan (and with it the `acknowledgement_failure` event type) and the `[ack: <rule_id>]` receipt token. Either requires a new `/codify` proposal to wire.
