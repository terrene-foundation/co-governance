...
> **Implementation status (read first).** The enforcement engine is LIVE (full enforcement wired in this repo — `rules/trust-posture.md` § Enforcement Status); the shapes below are the IMPLEMENTED on-disk shapes, pinned by `.claude/audit-fixtures/posture-engine/run-fixtures.js`. Two designed components were deliberately excluded from the GH #16 engine build (issue re-scope trail, 2026-06-10) and have NO mechanical implementation: the `stop.js` self-confession scan (and with it the `acknowledgement_failure` event type) and the `[ack: <rule_id>]` receipt token. Either requires a new `/codify` proposal to wire.
