...
> **Mode guard (read first — GATE-B).** This skill is the *how* of the graduated-trust system and documents BOTH enforcement modes. A **full-enforcement** repo (co-template) wires `/cc-audit` step-15 adjudication, so probe-confirmed cumulative downgrade, the grace `regression_within_grace` auto-drop, and the posture-engine fixture suite are all live. A **hooks-only/advisory** repo — like this one — runs the record + surface + deny-protection subset only: violations are recorded to `violations.jsonl` and surfaced in the `session-start.js` banner, but no probe adjudication or posture auto-downgrade runs, and the posture-engine fixture suite is not present until the upgrade path is taken. Read every "LIVE (GH #16, landed)", step-15, and posture-engine reference below as scoped to that distinction (`rules/trust-posture.md` § Enforcement Status and § Upgrade Path).
...
Origin: inbound sync from atelier (item-7 hook-cluster propagation, GH #15) — adapts atelier's trust-posture skill family, carrying the hook substrate (`detect-violations.js`, `lib/posture.js`) and CO v1.2 phase commands (`/codify`, `/vet`). Landed in hooks-only/advisory mode (GATE-B) — see the mode guard at the top.
