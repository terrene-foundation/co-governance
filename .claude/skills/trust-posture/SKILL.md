...
Origin: inbound sync from atelier (item-7 hook-cluster propagation, GH #15) — adapts atelier's trust-posture skill family, carrying the hook substrate (`detect-violations.js`, `lib/posture.js`) and CO v1.2 phase commands (`/codify`, `/vet`). Landed in full-enforcement mode (GATE-B) — see the enforcement-mode note at the top.
